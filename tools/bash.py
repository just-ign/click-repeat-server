...
class _BashSession:
    ...
    async def start(self):
        ...
        # Prepare environment with macOS considerations
        env = os.environ.copy()
        
        # Add homebrew paths on macOS if they exist and aren't already in PATH
        ...
        self._process = await asyncio.create_subprocess_shell(
            self.command,
            preexec_fn=os.setsid if os.name != 'nt' else None,  # setsid not available on Windows
            shell=True,
            bufsize=0,
            stdin=asyncio.subprocess.PIPE,
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE,
            env=env,  # Use our customized environment
        )
